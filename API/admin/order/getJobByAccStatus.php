...
$tsql = " SELECT  rowJss.*,convert(date,rowJss.job_date_sendTo,104) AS date_sendTo,tbd.*,tbp.paper,tbp.job_id AS p_job_id,tbp.job_id AS p_job_id
FROM    ( SELECT    ROW_NUMBER() OVER ( ORDER BY job_id DESC) AS RowNum, *
FROM tblJob
WHERE  job_status ='4' OR job_status ='40' AND job_delete_status = '0'
) AS rowJss
